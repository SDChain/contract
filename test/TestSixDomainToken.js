var SixDomainToken = artifacts.require("SixDomainToken");

contract('SixDomainToken', function(accounts) {
  it("should not be able to start ico sale when not called by owner.", function() {
    console.log("\n" + "-".repeat(100) + "\n");
    var sixdomainchain;
    var target;
    return SixDomainToken.deployed().then(function(instance) {
      sixdomainchain = instance;
      console.log("sixdomainchain:", sixdomainchain.address);
      return sixdomainchain.target.call({from: accounts[1]});
    }).then(function(t){
      target = accounts[1];
      web3.eth.sendTransaction({from: accounts[1], to: sixdomainchain.address, value: web3.toWei(1) })
      return web3.eth.sendTransaction({from: accounts[1], to: target, value: web3.toWei(1) })
    }).then(function(tx) {
      console.log("tx:", tx);
      console.log("blockNumber:", web3.eth.blockNumber);
      return sixdomainchain.start(100, {from: accounts[1]});
    }).then(function(tx) {
      console.log("tx:", tx);
      if (tx.logs) {
  ...
  it("should not allow create tokens before it starts", function() {
    ...
    return SixDomainToken.deployed().then(function(instance) {
      ...
      return web3.eth.sendTransaction({from: accounts[1], to: sixdomainchain.address, value: web3.toWei(1) });
  ...
  it("should be able to start ico sale when called by owner.", function() {
    ...
    return SixDomainToken.deployed().then(function(instance) {
      sixdomainchain = instance;
      return sixdomainchain.target.call();
    }).then(function(t){
      target = t;
      return sixdomainchain.start(10, {from: target});
    }).then(function(tx) {
      console.log("tx: ", tx);
      if (tx.logs) {
        for (var i = 0; i < tx.logs.length; i++) {
  ...
  it("should be able to create Loopring tokens after sale starts", function() {
    ...
    return SixDomainToken.deployed().then(function(instance) {
      ...
      target = t;
      return web3.eth.sendTransaction({from: accounts[1], to: sixdomainchain.address, value: web3.toWei(1), gas: 500000 });
    }).then(function(tx) {
      console.log("tx:", tx);
      return sixdomainchain.balanceOf(accounts[1], {from: accounts[1]});
    }).then(function(bal) {
  ...
  //     return sixdomainchain.computeTokenAmount(web3.toWei(1));
  ...
  //     return sixdomainchain.computeTokenAmount(web3.toWei(2.19));
  ...
  //     return sixdomainchain.computeTokenAmount(web3.toWei(3.04598));
  ...
  //     return sixdomainchain.computeTokenAmount(web3.toWei(1237.472));
  ...
  //     return sixdomainchain.computeTokenAmount(web3.toWei(0.01));
...
//     }).then(function(t){
//       target = t;
//       return sixdomainchain.ethGoalPerPhase.call({from: accounts[1]});
//     }).then(function(goal){
...
//     }).then(function(tx) {
//       console.log("txHash 1: ", tx);
//       return sixdomainchain.computeTokenAmount(web3.toWei(12.19), {from: accounts[1]});
//     }).then(function(amount1){
//       console.log("amount1:", amount1);
...
//     }).then(function(tx) {
//       console.log("txHash 1: ", tx);
//       return sixdomainchain.tokenAmountForOwner({from: accounts[1]});
//     }).then(function(amount1){
//       console.log("amount1:", amount1);
